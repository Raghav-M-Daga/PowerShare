...
class RequestsYouAccepted extends StatefulWidget {
  ...
  RequestsYouAccepted(Competition competition) {
  ...
  @override
  _RequestsYouAcceptedState createState() => _RequestsYouAcceptedState();
}

class _RequestsYouAcceptedState extends State<RequestsYouAccepted> {
  ...
  void navigateToPage(int index) {
    ...
    if (index == 3) {
      Navigator.pushReplacement(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation1, animation2) => AllRequests(new Competition("November 12th Qualifying Tournament", "12/10/22", "NorCal", true)),
  ...
  Widget generateOthersRequestsWidget(
      AsyncSnapshot<QuerySnapshot> snapshot, double width, double height) {
    List<Request> othersRequests = [];
    ...
    for (var req in data) {
      ...
      if (newRequestObj.teamRequesting != this.teamNumber &&
          newRequestObj.teamFulfilling == "") {
        othersRequests.add(newRequestObj);
        LocalNoticeService()
            .addNotification("New Request Has Been Added", "", "");
      }
    }
    ...
    //       child: Padding(
    //         padding: EdgeInsets.fromLTRB(
    //             width * 0.005, height * 0.0025, width * 0.005, height * 0.0025),
    //         child: Container(
    //           width: width * 0.3,
    //           height: height * 0.1,
    //           alignment: Alignment.center,
          ...
          child: Container(
            ...
            child: Padding(
              ...
              child: ListView.builder(
                ...
                itemCount: 1,
                itemBuilder: (BuildContext context, int i) {
                  // String request = othersRequests[i].getName();
                  // String teamRequesting = othersRequests[i].getRequestingTeam();
                  // if (othersRequests[i].teamFulfilling != "") {
                  //   isValidated = true;
                  // }
                  return Padding(
                    padding: EdgeInsets.fromLTRB(width * 0.02, height * 0.01,
                        ...
                        children: <Widget>[
                          Column(
                            children: [
                              Padding(
                                padding:
                                EdgeInsets.fromLTRB(width * 0.05, height * 0.008, 0, 0),
                                child: FittedBox(
                                    fit: BoxFit.scaleDown,
                                    child: Text("Mecanum Wheel",
                                        style: TextStyle(
                                            fontSize: 25.0,
                                            fontWeight: FontWeight.bold,
                                            color: Colors.white))),
                          ...
                          Padding(
                            padding: EdgeInsets.fromLTRB(width * 0.03, 0, width * 0.05, 0),
                          ...
                          //       }
                          //       if (!isValidated) {
                          //         return generateAcceptedStatusWidget(
                          //             othersRequests[i],
                          //             snapshot,
                          //             teamRequesting,
                          //             width,
                          //             height);
                          //       } else {
                          //         othersRequests.remove(othersRequests[i]);
                          //         return Container();
                          //       }
                          //     },
                          //   ),
  ...
  Widget generateAcceptedByYouWidget(
      AsyncSnapshot<QuerySnapshot> snapshot, double width, double height) {
    List<Request> acceptedByYou = [];
    ...
    for (var req in data) {
      ...
      if (newRequestObj.teamRequesting == this.teamNumber) {
        if (newRequestObj.teamFulfilling != "") {
          acceptedByYou.add(newRequestObj);
        }
      }
    }

    if (acceptedByYou.length == 0) {
      return Padding(
        padding: EdgeInsets.fromLTRB(
            width * 0.0375, height * 0.0375, width * 0.0375, height * 0.0375),
        child: Container(
          width: width * 0.35,
          child: Padding(
            padding: EdgeInsets.fromLTRB(
                width * 0.01, height * 0.01, width * 0.01, height * 0.01),
            child: Text(
              "None of your requests are accepted",
              textAlign: TextAlign.center,
              style: TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
          ...
          padding: EdgeInsets.fromLTRB(
              width * 0.05, height * 0.0125, width * 0.05, height * 0.0125),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.all(Radius.circular(10.0)),
            color: Color(0xff3E5CA2),
          ),
        ...
        child: SingleChildScrollView(
          child: Container(
            ...
            child: Padding(
              ...
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: acceptedByYou.length,
                itemBuilder: (BuildContext context, int i) {
                  String request = acceptedByYou[i].getName();
                  String teamRequesting = acceptedByYou[i].teamFulfilling;
                  return Padding(
                    padding: EdgeInsets.fromLTRB(width * 0.02, height * 0.01,
                        width * 0.02, height * 0.01),
                    child: new Container(
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          FittedBox(
                              fit: BoxFit.scaleDown,
                              child: Text("$teamRequesting has accepted",
                                  style: TextStyle(
                                      fontSize: 15.0,
                                      fontWeight: FontWeight.bold,
                                      color: Colors.white))),
                          SizedBox(
                            height: height * 0.01625,
                          ),
                          Container(
                            child: StreamBuilder<QuerySnapshot>(
                              stream: FirebaseFirestore.instance
                              ...
                              builder: (BuildContext context,
                                ...
                                return GestureDetector(
                                  ...
                                  onTap: () async {
                                    ...
                                    Navigator.pop(context);
                                    if (acceptedByYou[i].teamFulfilling != "") {
                                      acceptedCount--;
                          ...
                          Padding(
                            ...
                            child: StreamBuilder<QuerySnapshot>(
                              ...
                              builder: (BuildContext context,
                                ...
                                return generateAcceptedByYouWidget(
                                    snapshot,
